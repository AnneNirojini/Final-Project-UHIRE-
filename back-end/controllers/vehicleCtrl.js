...
const vehicleCtrl = {
    ...
    createVehicle: async(req, res) =>{
        try {
            ...
            const {vehicle_id, vehicleType, ServiceType, model, name, eMail, contactNumber, address} = req.body;
            ...
            const newVehicle = new Vehicles({
                vehicle_id, vehicleType, ServiceType, model, name, eMail, contactNumber, address
            })
    ...
    updateVehicle: async(req, res) =>{
        try {
            const {vehicleType, ServiceType, model, images, name, eMail, contactNumber, address} = req.body;
            // if(!images) return res.status(400).json({msg: "No image upload"})

            await Vehicles.findOneAndUpdate({_id: req.params.id}, {
                vehicleType, vehicleType, ServiceType, model, images, name, eMail, contactNumber, address
            })
